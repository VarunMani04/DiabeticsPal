...
def model_wrapper(x):
    ...
    model.eval()  # disable Batch Normalization during inference
    with torch.no_grad():
...
background_data = train_x_scaled[:100]  # Using first 100 samples

# SHAP explainer
explainer = shap.KernelExplainer(model_wrapper, background_data)

instance_to_explain = test_x_scaled[0:1]  # first test instance
...
# Get feature importances (absolute mean SHAP values)
...
sorted_indx = np.argsort(feature_importances)
feature_importances = feature_importances[sorted_indx]
feature_names = feature_names[sorted_indx]

# Create bar plot
plt.figure(figsize=(10, 6))
plt.barh(feature_names, feature_importances, color = 'royalblue')
plt.xlabel("Mean Absolute SHAP Value")
plt.ylabel("Features")
plt.title("BPNN Feature Importance")
plt.tight_layout()
plt.show()